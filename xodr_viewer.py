...
from elements import road, geometry
...
class XodrViewer:
    ...
    def draw_geometry_color(self):
        for road in self.xodr.roads:
            for geometry in road.planView.get_geometries:
                if isinstance(geometry, geometry.Line):
                    self._draw_line(geometry)
                elif isinstance(geometry, geometry.Arc):
                    self._draw_arc(geometry)
                elif isinstance(geometry, geometry.Spiral):
                    self._draw_spiral(geometry)
                elif isinstance(geometry, geometry.ParamPoly3):
                    self._draw_param_poly3(geometry)
                elif isinstance(geometry, geometry.Poly3):
    ...
    def _draw_spiral(self,spiral:geometry.Spiral):
        x = []
        y = []
    ...
    def _draw_param_poly3(self,poly:geometry.ParamPoly3):
        x = []
        y = []
    ...
    def _draw_poly3(self,poly:geometry.Poly3):
        x = []
        y = []
